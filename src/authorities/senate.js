...
/* ==========================================================================
   SENATE MEMBERS ENDPOINTS (UUID Integration)
   ========================================================================== */
...
router.post('/members', async (req, res) => {
  try {
    ...
    if (id && UUID_REGEX.test(id)) {
      query = `
        INSERT INTO senate_members (id, name, designation, affiliation, position, email, contact_phone)
        VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *
      `;
      params = [id, name, designation, affiliation, position, email, contactPhone];
    } else {
      query = `
        INSERT INTO senate_members (name, designation, affiliation, position, email, contact_phone)
...
router.post('/minutes', uploadFile, async (req, res) => {
  ...
  try {
    ...
    if (!title || !date) {
      ...
      return res.status(400).json({ error: 'Missing metadata: Title and Date are required.' });
    }
    ...
    res.status(500).json({ error: 'Failed to log meeting minutes.' });
  }
});
...
router.delete('/minutes/:id', validateUuid, async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Meeting minutes and associated document successfully deleted.' });
  } catch (err) {
    console.error('DELETE /minutes/:id error:', err);
    res.status(500).json({ error: 'Failed to wipe minutes record.' });
  }
});
