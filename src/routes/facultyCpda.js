...
router.post('/list', async (req, res) => {
    const { sl_no, particulars_en, particulars_hn, pdf_url, word_url } = req.body;
    try {
        const result = await pool.query(
            'INSERT INTO faculties_cpda_list (sl_no, particulars_en, particulars_hn, pdf_url, word_url) VALUES ($1, $2, $3, $4, $5) RETURNING *',
            [sl_no || '', particulars_en, particulars_hn, pdf_url, word_url]
        );
        res.json(result.rows[0]);
    } catch (err) {
        console.error(err);
...
router.put('/list/:id', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'UPDATE faculties_cpda_list SET sl_no=$1, particulars_en=$2, particulars_hn=$3, pdf_url=$4, word_url=$5 WHERE id=$6 RETURNING *',
            [sl_no || '', particulars_en, particulars_hn, pdf_url, word_url, id]
        );
        res.json(result.rows[0]);
    } catch (err) {
        console.error(err);
