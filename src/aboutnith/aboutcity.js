...
exports.create = async (req, res) => {
  try {
    const data = req.body;
    const fieldsArr = ['icon', 'title_en', 'title_hi', 'description_en', 'description_hi', 'image_url'];
    ...
    const query = 'INSERT INTO about_nith_city_info (' + keys.join(', ') + ') VALUES (' + keys.map((_, i) => '$' + (i + 1)).join(', ') + ') RETURNING *';
...
exports.update = async (req, res) => {
  try {
    ...
    const vals = keys.map(k => data[k]);
    vals.push(id);
    const query = 'UPDATE about_nith_city_info SET ' + setString + ' WHERE id = $' + vals.length + ' RETURNING *';
    const result = await sql.query(query, vals);
