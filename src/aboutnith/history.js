...
exports.getAll = async (req, res) => {
  try {
    const data = await sql.query('SELECT * FROM about_nith_timeline ORDER BY id ASC');
...
exports.create = async (req, res) => {
  try {
    const data = req.body;
    const fieldsArr = ['year', 'title_en', 'title_hi', 'description_en', 'description_hi'];
    ...
    const query = 'INSERT INTO about_nith_timeline (' + keys.join(', ') + ') VALUES (' + keys.map((_, i) => '$' + (i + 1)).join(', ') + ') RETURNING *';
...
exports.update = async (req, res) => {
  try {
    ...
    const vals = keys.map(k => data[k]);
    vals.push(id);
    const query = 'UPDATE about_nith_timeline SET ' + setString + ' WHERE id = $' + vals.length + ' RETURNING *';
    const result = await sql.query(query, vals);
