...
exports.create = async (req, res) => {
  try {
    const data = req.body;
    const fieldsArr = ['icon', 'title_en', 'title_hi', 'nearest_point_en', 'nearest_point_hi', 'distance_en', 'distance_hi', 'travel_time_en', 'travel_time_hi', 'services_en', 'services_hi', 'additional_info_en', 'additional_info_hi'];
    ...
    const query = 'INSERT INTO about_nith_connectivity_modes (' + keys.join(', ') + ') VALUES (' + keys.map((_, i) => '$' + (i + 1)).join(', ') + ') RETURNING *';
...
exports.update = async (req, res) => {
  try {
    ...
    const vals = keys.map(k => data[k]);
    vals.push(id);
    const query = 'UPDATE about_nith_connectivity_modes SET ' + setString + ' WHERE id = $' + vals.length + ' RETURNING *';
    const result = await sql.query(query, vals);
