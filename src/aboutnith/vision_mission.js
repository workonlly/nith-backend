...
exports.create = async (req, res) => {
  try {
    ...
    const query = 'INSERT INTO about_nith_missions (' + keys.join(', ') + ') VALUES (' + keys.map((_, i) => '$' + (i + 1)).join(', ') + ') RETURNING *';
...
exports.update = async (req, res) => {
  try {
    ...
    const vals = keys.map(k => data[k]);
    vals.push(id);
    const query = 'UPDATE about_nith_missions SET ' + setString + ' WHERE id = $' + vals.length + ' RETURNING *';
    const result = await sql.query(query, vals);
