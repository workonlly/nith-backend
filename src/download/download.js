...
router.post("/data", fileUploads, async (req, res) => {
  try {
    ...
    if (req.files && req.files["pdf_file"]) {
      ...
      file_url = pdf.location.startsWith("http")
        ? pdf.location
        : `${process.env.AWS_ENDPOINT_URL_S3}${pdf.location}`;
    }

    if (req.files && req.files["word_file"]) {
      const word = req.files["word_file"][0];

      word_url = word.location.startsWith("http")
        ? word.location
        : `${process.env.AWS_ENDPOINT_URL_S3}${word.location}`;
    }
...
router.delete("/data/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const records =
      await sql`SELECT file_url, word_url FROM download_tables WHERE id = ${id}`;
...
router.post("/meta", async (req, res) => {
  try {
    const { page_type, heading_en, heading_hi, subheading_en, subheading_hi } =
      req.body;
    const inserted = await sql`
